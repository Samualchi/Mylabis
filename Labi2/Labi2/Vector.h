...
protected:
    ...
    int start_index_v{}; // index first element
...
public:
    ...
    int get_start_index_v() const { return start_index_v; } // index first element
    TypeVal& operator[](int pos);             // access
    bool operator==(const Vector<TypeVal>& v) const;  // if equal
    bool operator!=(const Vector<TypeVal>& v) const;  // if not equal
    Vector<TypeVal>& operator=(const Vector& v);     // assign

    // scalar operations
    Vector  operator+(const TypeVal& val);   // sum scalar
    Vector  operator-(const TypeVal& val);   // difference scalar
    Vector  operator*(const TypeVal& val);   // multiply scalar

    // vector operations
    Vector  operator+(const Vector<TypeVal>& v);     // sum
    Vector  operator-(const Vector<TypeVal>& v);     // difference
    TypeVal  operator*(const Vector<TypeVal>& v);     //  multiply
